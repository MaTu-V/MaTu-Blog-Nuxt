  ...
  >
    <Form ref="articleForm" :model="articleForm" :rules="articleRuleValidate" :label-width="80">
      ...
      <div class="bl-text-center">
        <Button type="primary" @click="publishArticle('articleForm')">保 存</Button>
        ...
        form.validate((valid) => {
          if (valid) {
            if (that.content !== '') {
              ...
              that.$axios.post('/article/updArticle', {
                id: that.article.id,
                ...
                if (code === 200) {
                  ...
                  if (label !== null || label !== undefined || label.length !== 0){
                    // 存在标签 插入标签
                    that.$axios.get('/article/updArticleLabel', {
                  ...
                  // 清空表单
                  that.$Message.success('保存成功,页面刷新中!');
