  ...
  <div class="forget">
    <Form ref="phoneForm" v-if="switchFormStatus" :model="phoneForm" :rules="phoneRuleValidate" :label-width="80">
      ...
      <FormItem class="bl-button">
        <Button type="success" :loading="loadingFlag" @click="phoneSubmit(`phoneForm`)"> 验 证</Button>
      </FormItem>
    </Form>
    <Form ref="passwordForm" v-else :model="passwordForm" :rules="passwordRuleValidate" :label-width="80">
      ...
      <FormItem class="bl-button">
        <Button type="success" :loading="loadingFlag" @click="pwdSubmit(`passwordForm`)"> 修 改</Button>
      </FormItem>
    </Form>
  </div>
</template>
      ...
      getCode(name) {
        ...
        that.$refs[name].validateField('phone', (valid) => {
          if (!valid) {
            let phone = that.encrypt(that.$refs[name].model.phone)
            // 获取验证码
            that.$axios.post('/sms/sendForgetSms', {phone: phone})
              .then(({data: {code}}) => {
                if (code === 200) {
